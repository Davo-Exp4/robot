...
class MotorDriver():
    def __init__(self, debug=False, default=False):
        self.debug = debug
        self.default = default
        if (self.default):
            self.pwm = PCA9685(address=0x47) 
        else:
            self.pwm = PCA9685()
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("motor B forward, speed 50%, Run for 4S, then stop")
        m2.MotorRun('MB', 'forward', 50, 4)
        print("motor B forward, speed 50%, Run for 4S, then stop")
        m2.MotorRun('MB', 'backward', 50, 4)
        time.sleep(2)
